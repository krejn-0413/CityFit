<template>
  <div class="w-full bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
    <div class="text-xs text-gray-400 mb-2 flex items-center justify-between">
      ...
      <span v-if="totalCities > 0" class="text-gray-400">{{ totalCities }} 座城市</span>
    </div>

    <div v-if="mapState === 'loading'" class="w-full flex flex-col items-center justify-center gap-3" style="height: 400px">
      <div class="w-8 h-8 border-3 border-gray-200 border-t-gray-400 rounded-full animate-spin"></div>
      <span class="text-sm text-gray-400">地图数据加载中...</span>
    </div>

    <div v-else-if="mapState === 'error'" class="w-full flex flex-col items-center justify-center gap-4" style="height: 400px">
      <span class="text-4xl">🗺️</span>
      <span class="text-sm text-gray-500 text-center px-8">地图数据加载失败，请检查网络连接</span>
      <span class="text-xs text-gray-400 text-center px-8">{{ errorMessage }}</span>
      <button @click="retryLoadMap"
        class="px-5 py-2 bg-gray-100 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-200 transition-all"
      >
        🔄 重新加载
      </button>
    </div>
  ...
  try {
    ...
    chart.setOption({
      ...
      series: [{
        ...
        emphasis: {
          label: { show: true, color: '#374151', fontSize: 11, fontWeight: 500 },
          itemStyle: { borderColor: '#374151', borderWidth: 2 },
        },
      }],
      geo: {
        map: 'china',
